...
namespace Nwn.Tga
{
...

    {
        ...
        private BitmapSource? bmpTargaImage = null;
        private BitmapSource? bmpImageThumbnail = null;
        ...
        private void LoadTGAHeaderInfo(BinaryReader binReader)
        {
            if (binReader != null && binReader.BaseStream != null && binReader.BaseStream.Length > 0 && binReader.BaseStream.CanSeek == true)
            {
                try
                {
                    ...
                    {
                        byte[] ImageIDValueBytes = binReader.ReadBytes(this.objTargaHeader?.ImageIDLength ?? 0);
                                ...
                                for (int i = 0; i < this.objTargaHeader?.ColorMapLength; i++)
                                {
                                    ...
                                    switch (this.objTargaHeader?.ColorMapEntrySize)
                                    {
                                        ...
                                        case 24:
                                            ...
                                            this.objTargaHeader?.ColorMap.Add(Color.FromArgb(255, r, g, b));
                                            break;
                                        case 32:
        ...
        private void LoadTGAExtensionArea(BinaryReader binReader)
        {
            if (binReader != null && binReader.BaseStream != null && binReader.BaseStream.Length > 0 && binReader.BaseStream.CanSeek == true)
            {
                ...
                if ((this.objTargaFooter?.ExtensionAreaOffset > 0) && (this.objTargaExtensionArea != null))
                {
                    try
                    {
                        ...
                        if (this.objTargaExtensionArea.ColorCorrectionOffset > 0)
                        {
                            ...
                            {
                                a = (byte)binReader.ReadInt16();
                                r = (byte)binReader.ReadInt16();
                                b = (byte)binReader.ReadInt16();
                                g = (byte)binReader.ReadInt16();
                    ...
                    using (msData = new MemoryStream())
                    {
                        ...
                        for (int i = 0; i < rows?.Count; i++)
                        {
                            if (rows != null && rows[i] != null)
                            {
                                ...
                                byte[] brow = rows[i]?.ToArray() ?? Array.Empty<byte>();
        ...
        private void LoadTGAImage(BinaryReader binReader)
        {
            ...
            if ((this.objTargaHeader == null) || (objTargaFooter == null))
                return;
            ...
            var bytesPerPixel = (pf.BitsPerPixel + 7) / 8; // == 1 in this example
            var stride = bytesPerPixel * this.objTargaHeader.Width; // == width in this example
            this.bmpTargaImage = BitmapSource.Create(this.objTargaHeader.Width, this.objTargaHeader.Height, 96, 96, GetPixelFormat(), null, bimagedata, stride);
            ...
            if (this.objTargaHeader.ColorMap.Count > 0)
            {
                ...
                BitmapPalette pal = this.bmpTargaImage.Palette;
                ...
                for (int i = 0; i < this.objTargaHeader.ColorMap.Count; i++)
                {
                    ...
                    if (this.Format == TGAFormat.NEW_TGA && (this.objTargaFooter.ExtensionAreaOffset > 0) && (this.objTargaExtensionArea != null))
                    {
                        if (this.objTargaExtensionArea.AttributesType == 0 || this.objTargaExtensionArea.AttributesType == 1)
                            forceopaque = true;
                    }
                    else if ((this.Header != null) && (this.Header.AttributeBits == 0 || this.Header.AttributeBits == 1))
                            forceopaque = true;
                    
                    if (forceopaque)
                        // use 255 for alpha ( 255 = opaque/visible ) so we can see the image
                        pal.Colors.Add(Color.FromArgb(255, this.objTargaHeader.ColorMap[i].R, this.objTargaHeader.ColorMap[i].G, this.objTargaHeader.ColorMap[i].B));

                    else
                        // use whatever value is there
                        pal.Colors.Add(this.objTargaHeader.ColorMap[i]);
        ...
        {
            PixelFormat pfTargaPixelFormat = PixelFormats.Default;

            if ((this.objTargaHeader == null) || (this.objTargaFooter == null))
            ...
            {
                case 8:
                    pfTargaPixelFormat = PixelFormats.Indexed8;
                    break;
                ...
                case 16:
                    ...
                    if (this.Format == TGAFormat.NEW_TGA && (this.objTargaFooter.ExtensionAreaOffset > 0) && (this.objTargaExtensionArea != null))
                    {
                        switch (this.objTargaExtensionArea.AttributesType)
                        {
                            ...
                            case 2: // no alpha data
                                pfTargaPixelFormat = PixelFormats.Bgr555;
                                break;

                            case 3: // useful alpha data
                                pfTargaPixelFormat = PixelFormats.Bgr555; //  Format16bppArgb1555;
                                break;
                        }
                    }
                    else if (this.Header != null)
                    {
                        ...
                        if (this.Header.AttributeBits == 0)
                            pfTargaPixelFormat = PixelFormats.Bgr555; // Format16bppRgb555;
                ...
                case 24:
                    pfTargaPixelFormat = PixelFormats.Bgr24; //Format24bppRgb;
                    break;
                ...
                case 32:
                    ...
                    if (this.Format == TGAFormat.NEW_TGA && (this.objTargaFooter.ExtensionAreaOffset > 0) && (this.objTargaExtensionArea != null))
                    {
                        switch (this.objTargaExtensionArea.AttributesType)
                        {
                            ...
                            case 1:
                            case 2: // no alpha data
                                pfTargaPixelFormat = PixelFormats.Bgr32;// Format32bppRgb;
                                break;

                            case 3: // useful alpha data
                                pfTargaPixelFormat = PixelFormats.Bgra32; // Format32bppArgb;
                                break;

                            case 4: // premultiplied alpha data
                                pfTargaPixelFormat = PixelFormats.Pbgra32; // Format32bppPArgb;
                                break;
        ...
        private void LoadThumbnail(BinaryReader binReader, PixelFormat pfPixelFormat)
        {
            ...
            if (binReader != null && binReader.BaseStream != null && binReader.BaseStream.Length > 0 && binReader.BaseStream.CanSeek == true)
            {
                if (this.ExtensionArea != null && this.ExtensionArea.PostageStampOffset > 0)
                {
        ...
        public static BitmapImage? LoadTargaImage(string sFileName)
        {
        ...
        /// <returns>A Bitmap object with the Targa image loaded into it.</returns>
        public static BitmapImage? LoadTargaImage(Stream ImageStream)
        {
        ...
        private static BitmapImage? CopyToBitmap(TargaImage ti)
        {
            BitmapImage? b = null;
            //if (ti.Image != null)
            //{
    ...
    public class TargaHeader
    {
        ...
        /// </summary>
        public List<System.Windows.Media.Color> ColorMap
        {
